...
#define TIMEOUT 6 // Tempo limite para o cliente se logar
...
int main()
{
    ...
    int yes = 1;
    if (setsockopt(sock7, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1)
    {
        perror("setsockopt");
        exit(1);
    }
    ...
    yes = 1;
    if (setsockopt(sock8, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1)
    {
        perror("setsockopt");
        exit(1);
    }
    ...
    { // ------------------------------ while -----------------------------
        ...
        if ((client_sock7 = accept(sock7, (struct sockaddr *)&client_address, &len)) < 0)
        ...
        else
        {
            ...
            if ((client_sock8 = accept(sock8, (struct sockaddr *)&client_address, &len2)) < 0)
            ...
            else
            {
                ...
                printf("ID: %s\n", client_ip2);
                printf("%d\n", client_sock8);
                if ((strcmp(client_ip, client_ip2) == 0))
                { // Para garantir q o mesmo accept do 7 esta se conectando com o 8
                    send(client_sock8, "1", 1, 0);
                    count_client++;
                    controle_a = 1;
                }
                else
                { // Aceitou o 8, mas não são iguais
...
void remove_client(char *ip_address)
{
    ...
    {
        if (temp->nome[0] != '\0')
            controle = 1; // Ve se esta logado
        t = temp->thread;
        t_timeout = temp->thread_timeout;
...
{ // Cria cliente genérico
    ...
    if (pthread_create(&(new_node->thread), NULL, handle_client, (void *)new_node) < 0)
    {
     ...
     if (pthread_create(&(new_node->thread_timeout), NULL, timeout, (void *)new_node)< 0)
    {
        remove_client(new_node->ip_address);
        perror("Could not create thread");
        return false;
    }
...
bool refresh_client(char *ip_address, int sock7, int sock8)
{                            // Se um cliente tentar conexão mas ele já esta inserido
    ClientNode *temp = head; // Inicia o ponteiro temporário na cabeça da lista
    while (temp != NULL)
    { // Fecha sockets antigos, muda pelos novos e exclui tread antiga e faz uma nova
...
{ // Função para enviar um broadcast para todos os clientes na lista
    ...
    {
        if (temp->nome[0] != '\0')
        { // Manda pra todos os "Clientes logados"
...
void *timeout(void *node){
    ClientNode *cli = (ClientNode *)node;
    //--------------------- TIMEOUT -----------------------------------------
    while(1){ // Add condition to exit loop when client is removed
        if (cli->nome[0] == '\0' && cli->controle != 1){
            cli->end_time = current_time_in_seconds() + TIMEOUT; // Define o tempo limite para 60 segundos
            cli->controle = 1;
            printf("%f %f\n", current_time_in_seconds() , cli->end_time); // Debug
        }

        if (cli->nome[0] != '\0') cli->controle = 0;

        if(cli->controle == 1){
            while(cli->nome[0] == '\0'){
                if (current_time_in_seconds() >= cli->end_time)
                { // Se o cliente nao se logar em X segundos é automaticamente desconectado
                    printf("Time out do Cliente_ip: %s\nCliente %s desconectado", cli->ip_address, cli->ip_address);
                    fflush(stdout);
                    cli->controle = 0;
                    remove_client(cli->ip_address);
                    return NULL; // Exit function when client is removed
                }
            }
        }
    }
...
void *handle_client(void *node){
    ...
    // if (pthread_create(&(cli->thread_timeout), NULL, timeout, (void *)cli)< 0)
    // {
    ...
    while (1){
        ...
        { // Select esperar por atividade num dos sockets
            perror("Erro no select");
            exit(EXIT_FAILURE);
        }

        if (FD_ISSET(client_sock7, &readfds))
        {                                                                    // Gerenciamentode cliente na porta 50007 /lin/sai/out
            ...
            else
            {
                ...
                { // LIN ----------------------------
                    if ((cli->nome[0]) == '\0')
                    {
                        if (login_list_size() <= MAX_LOGIN)
                        { // Se não chegou no tamanho máximo (Definido como número máximo da lista 3).
                            if (name_not_in_list(nome))
                            {
                                printf("----------------> Inserção de cliente!\n");
                                strcpy(cli->nome, nome); // Salva o nome
                                print_logs();
                                if (send(client_sock7, "2", 1, 0) == -1)
                                { // Código 2 de controle para avisar q foi inserido
                                    printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                                    fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                                    remove_client(client_ip);
                            ...
                            { // Nome Já em uso
                                if (send(client_sock7, "9", 1, 0) == -1)
                                { // Código 9 de controle para avisar q o nome selecionado já esta em uso
                        ...
                        { // Lista cheia
                            if (send(client_sock7, "3", 1, 0) == -1)
                            { // Código 3 de controle para avisar q a lista esta cheia
                                printf("Falha na conexão com o cliente %s\n", client_ip);
                                fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                                remove_client(client_ip);
                                // exit(EXIT_FAILURE);
                    ...
                    { // Já esta na lista nome diferente de NULL
                        if (send(client_sock7, "4", 1, 0) == -1)
                        { // Código 4 pra avisa q já esta na lista (cliente <-7)
                            printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                            fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                            remove_client(client_ip);
                            // exit(EXIT_FAILURE);
                        ...
                        if (send(client_sock7, envio, strlen(envio) + 1, 0) == -1)
                        { // Retorna MSG pra avisar q ta na lista e o nome
                            printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                            fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                            remove_client(client_ip);
                            // exit(EXIT_FAILURE);
                ...
                { // Mensagem inválida -------------------------------------------
                    if (send(client_sock7, "0", 1, 0) == -1)
                    { // Código de controle 0 para avisar que o cliente mandou algo incongruente
                        printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                        fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                        remove_client(client_ip);
                    }
        ...
        if (FD_ISSET(client_sock8, &readfds))
        {                                                                    // Troca de mensagens na porta 50008 /msg/mpv/arq
            ...
            {
                if (cli->nome != NULL)
                { // So permite que o ciente envie mensagem se tiver nome
                    printf("Recebido na porta 8: %s\n", recebeporta8);
                    printf("Mensagem recebida de %s, com tamanho %d\n", client_ip, len);
                    ...
                    {                                                 // MPV -----------------------------------
                        ...
                        if (temp != NULL)
                        { // Função pra retorna o sock da conexão desse cliente  find_client_by_nome(nome)
                            sprintf(envio, "MPV <%s>: %s", cli->nome, mensagem);
                            if (send(temp->client_sock8, envio, strlen(envio) + 1, 0) == -1)
                            { // Envia a mensagem privada (cliente deseJádo <- 8)
                                printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                                fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                                remove_client(temp->ip_address);
                            }
                            if (send(client_sock7, "2", 1, 0) == -1)
                            { // Código 2 de controle avisar q enviou
                                printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                                fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                                remove_client(client_ip);
                            }
                        ...
                        {
                            if (send(client_sock7, "3", 1, 0) == -1)
                            { // Código 3 de controle avisar q nao enviou (cliente <- 8)
                                printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                                fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                                remove_client(client_ip);
                            }
                    ...
                    { // ARQ -----------------------------------
                        if (send(client_sock7, "2", 1, 0) == -1)
                        { // Código 2 de controle para o cliente comecar a enviar (cliente <- 8)
                            printf("Falha na conexão com o cliente %s <%s>\n", client_ip, cli->nome);
                            fflush(stdout); // Isso força a mensagem a aparecer no terminal imediatamente.
                            remove_client(client_ip);
                        }
...
void clear_client_list()
{ // Garante a lista n ter elementos
...
ClientNode *find_client_by_nome(const char *nome)
{ // Função pra retorna o sock da conexão desse cliente
    if (nome == NULL)
        return NULL;
    ClientNode *temp = head;
    while (temp != NULL)
    {
        if (strcmp(temp->nome, nome) == 0)
            return temp; // Compara nome dado com nome do ClientNode
        temp = temp->next;
    }
    return NULL;
}
bool name_not_in_list(const char *nome)
{                            // Procura se um nome jJá esta em uso
    ClientNode *temp = head; // Inicia o ponteiro temporário na cabeça da lista
    while (temp != NULL)
    { // Ve se o nome Já ta na lista
        if ((strcmp(temp->nome, nome)) == 0)
            return false;  // Compara nome
        temp = temp->next; // Move para o próximo nó
    }
...
bool segmenta_recebeporta(char *recebeporta, char *cod, char *nome, char *mensagem, int *tam, char *ext, char *nome_arq)
{
    if (strncmp(recebeporta, "/lin", 4) == 0)
    { // LIN ------------------------------------------------
        if (sscanf(recebeporta, "%4s %9s", cod, nome) == 2)
            return true;
    }
    if (strncmp(recebeporta, "/out", 4) == 0)
    { // OUT ------------------------------------------------
        strcpy(cod, "/out");
        return true;
    }
    if (strncmp(recebeporta, "/sai", 4) == 0)
    { // SAI ------------------------------------------------
        strcpy(cod, "/sai");
        return true;
    }
    if (strncmp(recebeporta, "/msg", 4) == 0)
    { // MSG ------------------------------------------------
        if (sscanf(recebeporta, "%4s %111s", cod, mensagem) == 2)
            return true;
    }
    if (strncmp(recebeporta, "/mpv", 4) == 0)
    { // MPV -----------------------------------------------
        if (sscanf(recebeporta, "%4s %9s %111s", cod, nome, mensagem) == 3)
            return true;
    }
    if (strncmp(recebeporta, "/arq", 4) == 0)
    { // ARQ -----------------------------------------------
        if (sscanf(recebeporta, "%4s %19s %9s %d", cod, nome_arq, ext, tam) == 4)
            return true;
    }
    return false;
}
...
bool dessegmenta(char *envio, const char *nome_arq, const char *ext, int tam)
{
    if (sprintf(envio, "%s %s %d", nome_arq, ext, tam) > 0)
        return true;
    return false;
}
